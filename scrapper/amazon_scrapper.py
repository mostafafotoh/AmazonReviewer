...
def scr(n, url):
    result = []
    if len(re.findall('[d,g]p/[A-Za-z0-9]+/', url)) == 0:
        base_url = re.findall('p/[A-Za-z0-9]+', url)
    elif re.findall('[d,g]p/[A-Za-z0-9]+/', url)[0] =="p/dp/":
        base_url = re.findall('[d,g]p/[A-Za-z0-9]+/', url)
    ...
    image_link_div = soup_prod("div", attrs={"id": "img-canvas"})
    image_link_soup = BeautifulSoup(str(image_link_div), "html.parser")
    image_link = image_link_soup.find('img')
    # print(image_link)
    image_link = image_link['data-a-dynamic-image']
    image_link = str(image_link).split("\"")
    ...
    return result, url_id, product_name, url_main, product_rate, image_link


def scrap_analyze(url, n=1):
    url_n = str(url)
    n = int(n)
    comments, url_id, product, url_ma, product_rate, image_link = scr(n, url=url_n)
    ...
    return rate, url_id, product, url_ma, product_rate, image_link
